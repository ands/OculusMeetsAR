...
Rift::Rift( int ID, Ogre::Root* root, Ogre::RenderWindow* renderWindow, bool rotateView )
{
	...
	mSceneMgr = root->createSceneManager(Ogre::ST_GENERIC, 1, Ogre::INSTANCING_CULLING_SINGLETHREAD);
	...
	mCamera = mSceneMgr->createCamera("OculusRiftExternalCamera"); mCamera->detachFromParent();
	...
	mViewport = mRenderWindow->addViewport(0, 0, mRenderWindow->getWidth(), mRenderWindow->getHeight()); // mCamera ); // TODO: andreas: is this correct?
	//mViewport->setBackgroundColour(Ogre::ColourValue::Black); // TODO: andreas: is this important?
...
void Rift::setCameras( Ogre::Camera* camLeft, Ogre::Camera* camRight )
{
	...
	Ogre::Viewport *lvp, *rvp;
	lvp = camLeft->getLastViewport();
	rvp = camRight->getLastViewport();

	renderTexture->addViewport(lvp->getLeft(), lvp->getTop(), lvp->getWidth(), lvp->getHeight());
	//renderTexture->getViewport(0)->setClearEveryFrame(true); // TODO: andreas: what about these?
	...
	renderTexture->addViewport(rvp->getLeft(), rvp->getTop(), rvp->getWidth(), rvp->getHeight());
